...
int8		turnflag	     		= 0 ;
uint8		turnbackspeed			= 42;//45   //？？？ 这个值是什么用？ 转弯时老在用
uint32  Acc0_temp=0;
...
//      acc_change(2236068);
	...
	__GmLeft.uiPulse = 89;//86
	__GmLeft.iSpeed = 80;
	__GmLeft.cState   = __MOTORRUN;
        
	while ((__GmLeft.uiPulse - __GmLeft.uiPulseCtr) > 3);//剩余步数大于3
	__GmRight.uiPulse  = (turnbackspeed - 40)/3 +9; //??? 这个又疑惑，不懂
	__GmRight.cState   = __MOTORRUN;
...
void objectGoTo (int8  cXdst, int8  cYdst, uint8 mode)
{
	uint8   i = 1, cL = 0, cR = 0, offset = 0;
        int jiasu=spurt_time*3;
        if(jiasu>6) jiasu=6;
        
	static uint8 gapflag = 0; //??? 这个参数没有看懂
        if((2136068-spurt_time*130000)>1900000) acc_change(2136068-spurt_time*100000);
        else acc_change(1950000);
