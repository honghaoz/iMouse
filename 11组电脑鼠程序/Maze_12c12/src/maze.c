...
uint8 ShieldLoad(uint8  ucDir)                              //在转弯函数中
{
    ...
    for(i=0;i<16;i++)
      for(j=0;j<16;j++)
      {
          if(GucMapBlock[i][j]==0)//如果没走过，FLAG设为全1（都有路走）
             GucMapFlag[i][j]=0xff;
          else
             GucMapFlag[i][j]=GucMapBlock[i][j];//否则相同
      }

    if(!uStartFlag)//标记起点X if（0）if（15）		//四角初始化
    {//如果起点是0
    ...
    for(j=1;j<16;j++)//从1开始时由于后面初始化用j-1                                 //四边初始化
    {
            GucMapLoad[0][j] = 0x01 | 0x04 |  ( (~GucMapFlag[0][j-1]) & 0x01)<<1 | ((~GucMapFlag[0][j])&0x04) >>1 ;
            //GucMapLoad[0][j] = 上面设为有墙 | 下面设为有墙 |  上面的意思是上下相邻的两格间的墙，其中有一个有墙则取有墙 ;            
            //地图左墙根据已读取的信息设置屏蔽信息
            //              上   下                             右
...
//                zlg7289Download(1, 5, 0, maze_time_ctr % 10);
    ...
    //           ucTemp++;
                if (ucRoadStat) 
                {                                                           /*  有可前进方向                */
                    crosswayChoice(); //crosswayChoice是法则选择，现在是中心法则    
                    if(ushieldflag == 1)
                    {
                       ushieldflag = 0;
                       n=delnode(n);
...
//                         zlg7289Download(0, 5, 0, GucYGoal);
                ...
                if(n!=0 && ( maze_time==37)){//(denggaozhi_temp>denggaozhi || maze_time==37)){
   ...
   //     zlg7289Download(0, 5, 0, 0x0E);
     //   zlg7289Download(0, 6, 0, 0x0E);
       // zlg7289Download(0, 7, 0, 0x0E);
          ...
          {
          case 1:
            objectGoTo(GucXGoal,GucYGoal,__END);                                            /*  运行到指定目标点            */
